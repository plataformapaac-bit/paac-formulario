...
// @supabase/supabase-js é a biblioteca oficial do Supabase para Node.js
// Ela permite que o servidor leia e grave dados no banco de São Paulo
...
// Serve script do painel admin como arquivo estático protegido
app.get('/painel/admin.js', exigirAdmin, function(req, res) {
...
function htmlLogin(msg) {
  ...
  return '<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>PAAC</title>' + fonts + '<style>' + cssBase + cssBgAzul + cssCard + cssHdrAzul + cssCampo + cssBtn + '.body{padding:24px 28px}.aviso{background:#fff8ec;border:1px solid rgba(201,168,76,.4);border-radius:10px;padding:10px 13px;font-size:.74rem;color:#7a5a1a;margin-bottom:16px;line-height:1.5}.aviso strong{display:block;margin-bottom:3px;color:#5a3a00}.msg{border-radius:10px;padding:8px 12px;font-size:.76rem;margin-bottom:12px}.fase{background:#fff8ec;border:1px solid rgba(201,168,76,.4);border-radius:10px;padding:7px 11px;font-size:.73rem;color:#7a5a1a;text-align:center;margin-top:12px}.link{text-align:center;margin-top:10px}.link a{color:#2a5298;font-size:.74rem}</style></head><body><div class="card"><div class="hdr"><div class="sigla">PAAC</div><div class="sub">Plataforma de Automacao e Auditoria Cartorial</div></div><div class="body"><div class="aviso"><strong>O que e a PAAC?</strong>A Plataforma de Automacao e Auditoria Cartorial transforma um processo manual de 8 a 16 horas na geracao de documentos de incorporacao imobiliaria em aproximadamente 1 hora.</div><div style="background:#e8f0fb;border:1px solid rgba(42,82,152,.2);border-radius:10px;padding:8px 13px;font-size:.72rem;color:#1a3a6b;margin-bottom:12px"><strong style="display:block;margin-bottom:2px">Fase E0 - Acesso Restrito</strong>Plataforma em testes. Acesso exclusivo para a equipe PAAC.</div>' + (msg ? '<div class="msg" style="' + corMsg + '">' + msg + '</div>' : '') + '<form method="POST" action="/login"><div class="campo"><label>E-mail</label><input type="email" name="email" placeholder="seu@email.com" required autofocus></div><div class="campo"><label>Senha</label>' + senhaInput('s1','senha','Sua senha (branco se 1o acesso)',false) + '<span style="font-size:.67rem;color:#6b7a99;margin-top:2px">Primeiro acesso? Deixe em branco.</span></div><button type="submit" class="btn">Entrar na Plataforma</button></form><div class="link"><a href="/emergencia">Esqueci minha senha (admin)</a></div><div class="fase">Fase E0 — Acesso para testes.</div></div></div>' + olhoScript + '</body></html>';
}
...
// ═══ ROTA /api/me — retorna dados do usuário logado ═══
// Usada pelo index.html para exibir o nome no header e iniciar o cronômetro
...
// Middleware auxiliar: exige login simples (qualquer usuário logado)
...
// GET /api/projetos — lista todos os projetos do usuário logado
...
// GET /api/projetos/:id — carrega os dados completos de um projeto
...
// POST /api/projetos — cria ou atualiza projeto (autosave)
// Se vier com "id" no body: atualiza. Se não vier: cria novo.
app.post('/api/projetos', exigirLogin, async function(req, res) {
  ...
  try {
    var resultado;
    if (id) {
      // Atualiza projeto existente — verifica se é do usuário antes
      var { data, error } = await supabase
        .from('projetos')
        .update({ nome_projeto: nome, dados: dados, atualizado_em: agora })
        .eq('id', id)
        .eq('usuario_email', req.usuarioEmail) // segurança: só dono altera
        .select()
        .single();
      if (error) throw error;
      resultado = data;
...
// DELETE /api/projetos/:id — exclui um projeto do usuário
app.delete('/api/projetos/:id', exigirLogin, async function(req, res) {
  ...
  try {
    var { error } = await supabase
      ...
      .delete()
      .eq('id', req.params.id)
      .eq('usuario_email', req.usuarioEmail); // segurança: só dono exclui
    if (error) throw error;
    res.json({ ok: true });
  } catch (e) {
