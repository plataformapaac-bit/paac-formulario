...
app.get('/api/me', function(req,res) {
  var s=lerSessao(req);
  if(!s)return res.status(401).json({erro:'Nao autenticado.'});
...
var cssCard = '.card{background:#fff;border-radius:16px;box-shadow:0 20px 60px rgba(0,0,0,.4);width:100%;max-width:440px;overflow:hidden}';
...
function htmlLogin(msg) {
  ...
  return '<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>PAAC</title>' + fonts + '<style>' + cssBase + cssBgAzul + cssCard + cssHdrAzul + cssCampo + cssBtn + '.body{padding:24px 28px}.intro{background:#fff8ec;border-bottom:1px solid rgba(201,168,76,.3);padding:14px 20px;font-size:.77rem;color:#7a5a1a;line-height:1.6}.intro strong{display:block;font-size:.81rem;margin-bottom:4px;color:#5a3a00}.aviso{background:#fff8ec;border:1px solid rgba(201,168,76,.4);border-radius:10px;padding:10px 13px;font-size:.74rem;color:#7a5a1a;margin-bottom:16px;line-height:1.5}.aviso strong{display:block;margin-bottom:3px;color:#5a3a00}.msg{border-radius:10px;padding:8px 12px;font-size:.76rem;margin-bottom:12px}.fase{background:#fff8ec;border:1px solid rgba(201,168,76,.4);border-radius:10px;padding:7px 11px;font-size:.73rem;color:#7a5a1a;text-align:center;margin-top:12px}.link{text-align:center;margin-top:10px}.link a{color:#2a5298;font-size:.74rem}</style></head><body><div class="card"><div class="hdr"><div class="sigla">PAAC</div><div class="sub">Plataforma de Automacao e Auditoria Cartorial</div></div><div class="intro"><strong>O que e a PAAC?</strong>A Plataforma PAAC produz automaticamente os documentos necessarios para o Registro Cartorial de Condominios Edilícios: <strong>NBR 12721 (Quadros I-VIII), Convencao de Condominio, Declaracao de Vagas de Garagem</strong> e <strong>Memorial de Incorporacao ou de Instituicao</strong> — com congruencia garantida de 100% entre todos os documentos. Tenha em maos as plantas e extracoes de areas antes de iniciar. O documento final devera ser revisado pelo Responsavel Tecnico antes do protocolo cartorial.</div><div class="body"><div class="aviso"><strong>Acesso restrito</strong>Informe seu e-mail e senha para acessar.</div>' + (msg ? '<div class="msg" style="' + corMsg + '">' + msg + '</div>' : '') + '<form method="POST" action="/login"><div class="campo"><label>E-mail</label><input type="email" name="email" placeholder="seu@email.com" required autofocus></div><div class="campo"><label>Senha</label>' + senhaInput('s1','senha','Sua senha (branco se 1o acesso)',false) + '<span style="font-size:.67rem;color:#6b7a99;margin-top:2px">Primeiro acesso? Deixe em branco.</span></div><button type="submit" class="btn">Entrar na Plataforma</button></form><div class="link"><a href="/emergencia">Esqueci minha senha (admin)</a></div><div class="fase">Fase E0 — Acesso para testes. Pagamento sera ativado apos validacao da plataforma.</div></div></div>' + olhoScript + '</body></html>';
}
